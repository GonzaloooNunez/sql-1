...
SELECT COUNT(*) AS mayores_30 FROM usuarios_lenguajes WHERE edad > 30;

-- Ejercicio 4: Contar cuántos usuarios saben JavaScript.
SELECT COUNT(*) AS lenguaje_Javascript FROM usuarios_lenguajes WHERE lenguaje = 'JavaScript';
...
SELECT * FROM usuarios_lenguajes WHERE id_usuario NOT IN (SELECT id_usuario FROM usuarios_lenguajes WHERE lenguaje IS NOT NULL);
...
SELECT nombre, apellido, edad FROM usuarios_lenguajes WHERE edad IN (SELECT edad FROM usuarios_lenguajes GROUP BY edad HAVING COUNT(*) > 1) ORDER BY edad ;


-- Ejercicio 26: Encontrar el usuario con el lenguaje con mayor número de carácteres y que tenga una edad menor de 30 años. 
SELECT nombre, edad, lenguaje FROM usuarios_lenguajes WHERE edad < 30 ORDER BY LENGTH(lenguaje) DESC LIMIT 1;

-- Ejercicio 27: Seleccionar los usuarios que tienen al menos un lenguaje asociado y mostrar sus emails.
SELECT email FROM usuarios_lenguajes GROUP BY email HAVING COUNT(DISTINCT lenguaje) = 1;


-- Ejercicio 28: Contar cuántos usuarios tienen un apellido que comienza con la letra 'G'.
SELECT COUNT(*) AS cantidad_usuarios_g FROM usuarios_lenguajes WHERE apellido LIKE 'G%';
...
SELECT nombre, apellido, edad, edad - AVG(edad) OVER() AS diferencia_edad_promedio FROM usuarios_lenguajes;
...
SELECT nombre, apellido, lenguaje FROM usuarios_lenguajes ORDER BY id_usuario DESC LIMIT 1;
